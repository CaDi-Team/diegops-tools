...
- `x86_64-pc-windows-gnu` (Windows terminal / WSL — GNU toolchain via Zig/MinGW)
...
- Self-hosted runner: **all** `runs-on` values must be `ubuntu-latest` — never use `ubuntu-latest`, `macos-latest`, or `windows-latest`
